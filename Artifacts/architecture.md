...
A lot of the content in our project is within highly independent minigames. This makes our project more modular and makes it easier to perform changes to our software. If we wish to change an individual minigame, then we can make changes within the minigame without considering the rest of our product. Otherwise, if we want to make a change to the overall structure of our game, then we can use the modular minigames in any way we want, since they all run independently and make no assumptions about their context.
